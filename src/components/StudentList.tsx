...
import { Trash2, CreditCard as Edit, Archive, ArchiveRestore } from 'lucide-react';
import { Student, getStudentStatus, getStatusColor } from '../lib/supabase';
...
export default function StudentList({ students, loading, onDelete, onUpdate, onArchive, showArchived = false }: StudentListProps) {
                ...
                <tbody>
                  {students.map((student) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-sm font-semibold text-gray-900">{student.total_faltas}</td>
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                const msg = showArchived
                                  ? 'Deseja restaurar este estudante para a lista ativa?'
                                  : 'Deseja arquivar este estudante?';
                                if (confirm(msg)) onArchive(student.id, !showArchived);
                              }}
                              className={`p-1 ${showArchived ? 'text-green-600 hover:text-green-700' : 'text-amber-500 hover:text-amber-600'}`}
                              title={showArchived ? 'Restaurar estudante' : 'Arquivar estudante'}
                            >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {onArchive && (
                        <button
                          ...
                          className={`p-2 ${showArchived ? 'text-green-600 hover:text-green-700' : 'text-amber-500 hover:text-amber-600'}`}
                          title={showArchived ? 'Restaurar estudante' : 'Arquivar estudante'}
                        >
                          {showArchived ? <ArchiveRestore className="w-5 h-5" /> : <Archive className="w-5 h-5" />}
                        </button>
