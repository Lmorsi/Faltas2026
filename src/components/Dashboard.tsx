...
import { GraduationCap, Users, FileText, LogOut, AlertTriangle, Plus, Menu, X, CheckCircle2, AlertCircle, Archive } from 'lucide-react';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  const [currentView, setCurrentView] = useState<'students' | 'archived' | 'reports'>('students');
  ...
  const handleArchiveStudent = async (id: string, archive: boolean) => {
    try {
      ...
      setErrorMessage(`Erro ao arquivar: ${error.message}`);
    }
  };
  ...
  const regularCount = activeStudents.filter(s => s.total_faltas <= 100).length;
  const atencaoCount = activeStudents.filter(s => s.total_faltas >= 101 && s.total_faltas <= 179).length;
  const criticoCount = activeStudents.filter(s => s.total_faltas >= 180).length;

  const filterStudents = (list: Student[]) => list.filter(student => {
    const matchesSearch = student.nome_completo.toLowerCase().includes(searchTerm.toLowerCase());

    let matchesSerie = true;
    if (filterSerie) {
      const term = filterSerie.toLowerCase().trim();
      ...
      matchesSerie = serieTurmaCompact.includes(term) ||
                     serieTurmaWithSpace.includes(term) ||
                     serieTurmaWithDegree.includes(term) ||
                     serieTurmaWithDegreeCompact.includes(term) ||
  ...
  const filteredStudents = filterStudents(activeStudents);
  const filteredArchivedStudents = filterStudents(archivedStudents);

  const sortList = (list: Student[]) => [...list].sort((a, b) => {
    switch (sortBy) {
      case 'nome':
  ...
  const sortedStudents = sortList(filteredStudents);
  const sortedArchivedStudents = sortList(filteredArchivedStudents);
          ...
          <div className="hidden lg:flex items-center gap-4">
            <nav className="flex gap-2">
              ...
              <NavButton
                ...
                label={`Arquivados${archivedStudents.length > 0 ? ` (${archivedStudents.length})` : ''}`}
              />
              <NavButton
        ...
        {showMobileMenu && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-[#3D4A5C] border-t border-gray-600 p-4 shadow-xl">
             <div className="flex flex-col gap-2">
                ...
                <button onClick={() => {setCurrentView('archived'); setShowMobileMenu(false)}} className="p-3 text-left hover:bg-gray-700 rounded flex items-center gap-2"><Archive className="w-4 h-4" />Arquivados {archivedStudents.length > 0 && `(${archivedStudents.length})`}</button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-4 sm:p-6 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-gray-800">Estudantes Arquivados</h2>
                    <p className="text-sm text-gray-500">Estudantes removidos da lista ativa. Restaure para reativá-los.</p>
                  </div>
                </div>
              </div>

              <div className="p-4 bg-gray-50/50 grid grid-cols-1 md:grid-cols-2 gap-4">
                <FilterInput label="Buscar Nome" placeholder="Ex: João Silva..." value={searchTerm} onChange={setSearchTerm} />
                <FilterInput label="Filtrar Série/Turma" placeholder="Ex: 8º A..." value={filterSerie} onChange={setFilterSerie} />
              </div>

              <StudentList
                students={sortedArchivedStudents}
                loading={loading}
                onDelete={handleDeleteStudent}
                onUpdate={loadStudents}
                onArchive={handleArchiveStudent}
